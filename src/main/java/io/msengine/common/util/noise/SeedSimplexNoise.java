 ...
 *
 */
public class SeedSimplexNoise {
	
	public static long RANDOM_SEED = 0L;
	public static int SWAP_COUNT   = 400;
	...
	private static final Vector3b[] grad3 = {
			new Vector3b(1, 1, 0), new Vector3b(-1, 1, 0), new Vector3b(1, -1, 0), new Vector3b(-1, -1, 0),
			new Vector3b(1, 0, 1), new Vector3b(-1, 0, 1), new Vector3b(1, 0, -1), new Vector3b(-1, 0, -1),
			new Vector3b(0, 1, 1), new Vector3b(0, -1, 1), new Vector3b(0, 1, -1), new Vector3b(0, -1, -1)
	};
	...
	private static final Vector4b[] grad4 = {
			new Vector4b(0, 1, 1, 1),  new Vector4b(0, 1, 1, -1),  new Vector4b(0, 1, -1, 1),  new Vector4b(0, 1, -1, -1),
			new Vector4b(0, -1, 1, 1), new Vector4b(0, -1, 1, -1), new Vector4b(0, -1, -1, 1), new Vector4b(0, -1, -1, -1),
			new Vector4b(1, 0, 1, 1),  new Vector4b(1, 0, 1, -1),  new Vector4b(1, 0, -1, 1),  new Vector4b(1, 0, -1, -1),
			new Vector4b(-1, 0, 1, 1), new Vector4b(-1, 0, 1, -1), new Vector4b(-1, 0, -1, 1), new Vector4b(-1, 0, -1, -1),
			new Vector4b(1, 1, 0, 1),  new Vector4b(1, 1, 0, -1),  new Vector4b(1, -1, 0, 1),  new Vector4b(1, -1, 0, -1),
			new Vector4b(-1, 1, 0, 1), new Vector4b(-1, 1, 0, -1), new Vector4b(-1, -1, 0, 1), new Vector4b(-1, -1, 0, -1),
			new Vector4b(1, 1, 1, 0),  new Vector4b(1, 1, -1, 0),  new Vector4b(1, -1, 1, 0),  new Vector4b(1, -1, -1, 0),
			new Vector4b(-1, 1, 1, 0), new Vector4b(-1, 1, -1, 0), new Vector4b(-1, -1, 1, 0), new Vector4b(-1, -1, -1, 0)
	};
	
	// This contains all the numbers between -128 and 127, in random order depending on seed.
	private static byte[] pSupply = { -105, -96, -119, 91, 90, 15, -125, 13, -55, 95, 96, 53, -62, -23, 7, -31, -116, 36, 103, 30, 69, -114, 8, 99, 37, -16,
	...
	private final byte[] p = new byte[pSupply.length];
	private final byte[] perm = new byte[512];
	private final byte[] permMod12 = new byte[512];
	
	public SeedSimplexNoise(long seed) {
		...
		for (int i = 0; i < SWAP_COUNT; i++) {
			...
			from = rand.nextInt(p.length);
			to   = rand.nextInt(p.length);
	...
	/**
	 * @return This simplex noise seed.
	 */
	public long seed() {
		return this.seed;
	}
	...
	// Normalized
	public float normnoise(float x, float y) {
		return (this.noise(x, y) + 1f) / 2f;
	}
	
	public float normnoise(float x, float y, float z) {
		return (this.noise(x, y, z) + 1f) / 2f;
	}
	
	public float normnoise(float x, float y, float z, float w) {
		return (this.noise(x, y, z, w) + 1f) / 2f;
	}
